...
#             '-gencode=arch=compute_86,code=sm_86',  # For new graphics cards
#             '--use_fast_math'
#         ]
...
def KaolinCUDAExtension(*args, **kwargs):
    if not os.name == 'nt':
        ...
        kwargs['extra_compile_args'].setdefault('cxx', [])
        kwargs['extra_compile_args'].setdefault('nvcc', [])

        kwargs['extra_compile_args']['cxx'] += cxx_flags
        kwargs['extra_compile_args']['nvcc'] += nvcc_flags
